...
#include "mediapipe/calculators/util/detections_to_text_calculator.pb.h"
...
// A calculator that converts Detection proto to Text proto for
// visualization.
//
...
//   }
// }
class DetectionsToTextCalculator : public CalculatorBase {
 public:
  DetectionsToTextCalculator() {}
  ~DetectionsToTextCalculator() override {}
  DetectionsToTextCalculator(const DetectionsToTextCalculator&) =
      delete;
  DetectionsToTextCalculator& operator=(
      const DetectionsToTextCalculator&) = delete;
 ...
 private:
  ...
  static void SetRenderAnnotationColorThickness(
      const DetectionsToTextCalculatorOptions& options,
  ...
  static void AddLabels(const Detection& detection,
                        const DetectionsToTextCalculatorOptions& options,
                        float text_line_height, std::string* render_data);
  ...
  static void AddDetectionToText(
      const Detection& detection,
      const DetectionsToTextCalculatorOptions& options,
      std::string* render_data);
...
REGISTER_CALCULATOR(DetectionsToTextCalculator);

::mediapipe::Status DetectionsToTextCalculator::GetContract(
...
::mediapipe::Status DetectionsToTextCalculator::Open(
...
::mediapipe::Status DetectionsToTextCalculator::Process(
    CalculatorContext* cc) {
  const auto& options = cc->Options<DetectionsToTextCalculatorOptions>();
  ...
  // DetectionsToTextCalculatorOptions.
  auto render_data = absl::make_unique<std::string>();

  //  render_data->set_scene_class(options.scene_class());
  *render_data += "{ \"stream\": \"";
  *render_data += options.scene_class();
  *render_data += "\", \"hands\" : [";

  if (has_detection_from_list) {
    for (const auto& detection :
         cc->Inputs().Tag(kDetectionListTag).Get<DetectionList>().detection()) {
      AddDetectionToText(detection, options, render_data.get());
    }
  }
  ...
  *render_data = render_data->substr(0, render_data->size() - 2);

  *render_data += "]},\n";

  output_file_ = fopen(output_file_path_.c_str(), "a+");
  if (output_file_ != NULL) {
      fwrite(render_data.get()->c_str(), 1, render_data.get()->size(), output_file_);
  }
  fclose(output_file_);
...
::mediapipe::Status DetectionsToTextCalculator::Close(
    CalculatorContext* cc) {
  output_file_ = fopen(output_file_path_.c_str(), "a+");
  if (output_file_) {
      fwrite("]\n}\n", 1, 4, output_file_);
...
void DetectionsToTextCalculator::SetRenderAnnotationColorThickness(
    const DetectionsToTextCalculatorOptions& options,
    RenderAnnotation* render_annotation) {
...
void DetectionsToTextCalculator::SetTextCoordinate(
...
void DetectionsToTextCalculator::SetRectCoordinate(
...
void DetectionsToTextCalculator::AddLabels(
    const Detection& detection,
    const DetectionsToTextCalculatorOptions& options,
    float text_line_height, std::string* render_data) {
  ...
  *render_data += "\"labels\": [";
  ...
  for (int i = 0; i < num_labels; ++i) {
    ...
    char buffer[200];
    sprintf(buffer, "{\"label\": \"%s\", \"score\": %f}", label_str.c_str(), rounded_score);
    *render_data += buffer;
  }
  *render_data += "],";
  ...
  for (int i = 0; i < labels.size(); ++i) {
    ...
    ::mediapipe::RenderAnnotation label_annotation_;
    auto* label_annotation = &label_annotation_;
...
void DetectionsToTextCalculator::AddFeatureTag(
    const Detection& detection,
    const DetectionsToTextCalculatorOptions& options,
    float text_line_height, std::string* render_data) {
  ::mediapipe::RenderAnnotation feature_tag_annotation_;
  auto* feature_tag_annotation = &feature_tag_annotation_;
  ...
  *render_data += "\"tag\": \"";
  *render_data += detection.feature_tag();
  *render_data += "\",";
}

void DetectionsToTextCalculator::AddLocationData(
    const Detection& detection,
    const DetectionsToTextCalculatorOptions& options,
    std::string* render_data) {
  ::mediapipe::RenderAnnotation location_data_annotation_;
  auto* location_data_annotation = &location_data_annotation_;
  ...
  if (detection.location_data().format() == LocationData::BOUNDING_BOX) {
    ...
    if (detection.location_data().relative_keypoints_size()) {
      for (int i = 0; i < detection.location_data().relative_keypoints_size();
           ++i) {
        ::mediapipe::RenderAnnotation keypoint_data_annotation_;
        auto* keypoint_data_annotation = &keypoint_data_annotation_;
  ...
  char buffer[200];
  sprintf(buffer, "\"left\": %f, \"right\": %f, \"bottom\": %f, \"top\": %f, \"rotation\": %f, \"normalized\": %d",
          location_data_rect->left(), location_data_rect->right(),
          location_data_rect->bottom(), location_data_rect->top(),
          location_data_rect->rotation(), location_data_rect->normalized());
  *render_data += "\"location\": { ";
  *render_data += buffer;
  *render_data += "}\n";
}

void DetectionsToTextCalculator::AddDetectionToText(
    const Detection& detection,
    const DetectionsToTextCalculatorOptions& options,
    std::string *render_data) {
